...
function detectLocales(localesDir) {
  if (!fs__namespace.existsSync(localesDir)) return [];
  return fs__namespace.readdirSync(localesDir).filter(f => f.endsWith(".json")).map(f => f.replace(".json", ""));
}
/**
 * 读取单个语言 JSON 文件，返回展平后的 key-value map
 */
function readLocaleJson(localesDir, locale) {
  const filePath = path__namespace.join(localesDir, `${locale}.json`);
 ...
 */
function readExistingExcel(excelPath, keyColumn) {
  const result = new Map();
  if (!fs__namespace.existsSync(excelPath)) return result;
  try {
    const fileBuffer = fs__namespace.readFileSync(excelPath);
 ...
 */
function jsonToExcel(options = {}, rootDir = process.cwd()) {
  const {
  ...
  const absLocalesDir = path__namespace.resolve(rootDir, localesDir);
  const absExcelPath = path__namespace.resolve(rootDir, excelPath);
  // 1. 确定要处理的语言列表
  ...
  if (highlightMissing) {
    ...
    for (let R = 1; R <= range.e.r; R++) {
      for (let C = 1; C <= range.e.c; C++) {
        const cellAddr = utils.encode_cell({
          r: R,
          c: C
        });
  ...
  // 确保输出目录存在
  const outputDir = path__namespace.dirname(absExcelPath);
  ...
  // fs.writeFileSync()
  fs__namespace.writeFileSync(absExcelPath, excelBuffer);
  // XLSX.writeFile(workbook, )
  console.log(`[i18n-excel] ✅ Excel 已生成: ${absExcelPath}`);
  console.log(`[i18n-excel] 📊 统计: 共 ${allKeysOrdered.length} 个 key，新增 ${newKeyCount} 个，缺失翻译 ${missingTranslationCount} 处`);
}
 ...
 * // → { tags: [4, 5] }
 */
const deepMerge = (target, source, mergeArray) => {
  const merged = {
    ...target
  };
  for (const key in source) {
    if (source.hasOwnProperty(key)) {
      ...
      if (isObject(sourceValue) && !Array.isArray(sourceValue)) {
        ...
        // 源值是数组：使用自定义回调或直接覆盖
        merged[key] = mergeArray ? mergeArray(Array.isArray(targetValue) ? targetValue : [], sourceValue) : sourceValue;
      } else {
        // 源值是基本类型：直接覆盖
 ...
 */
function parseExcel(excelPath, options) {
  const {
    sheetName = 0,
    keyColumn = "key",
    nestedKeys = true,
  ...
  if (!fs__namespace.existsSync(excelPath)) {
  ...
  if (locales.length === 0) {
    throw new Error(`[vite-plugin-i18n-excel] 没有找到语言列，请确认第一行包含语言标识（除 "${keyColumn}" 列外）`);
  }
  // 构建每个语言的翻译对象
  const result = new Map();
...
function writeLocaleFiles(translations, outputDir) {
  ...
  for (const [locale, data] of translations) {
    const filePath = path__namespace.join(outputDir, `${locale}.json`);
 ...
 * Vite 插件主体
 */
function i18nExcelPlugin(options = {}) {
  const {
    excelPath = "src/locales/translations.xlsx",
  ...
  let isBuild = false;
  let generated = false;
  const generate = rootDir => {
    const absExcelPath = path__namespace.resolve(rootDir, excelPath);
    const absOutputDir = path__namespace.resolve(rootDir, outputDir);
    try {
  ...
  return {
    ...
    configureServer(server) {
      ...
      const absExcelPath = path__namespace.resolve(root, excelPath);
      server.watcher.add(absExcelPath);
      server.watcher.on("change", changedPath => {
        if (path__namespace.resolve(changedPath) === absExcelPath) {
...
exports.default = i18nExcelPlugin;
exports.i18nExcelPlugin = i18nExcelPlugin;
